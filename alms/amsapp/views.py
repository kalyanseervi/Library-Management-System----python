...
def create_dealer(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('dealer_list')  # Redirect to a dealer list view
    else:
        form = Dealers2Form()
...
def update_dealer(request, dealer_id):
    ...
    return render(request, 'update_dealer.html', {'form': form, 'dealer': dealer})
...
def delete_dealer(request, dealer_id):
    ...
    if request.method == 'POST':
        dealer.delete()
        return redirect('dealer_list')
    return render(request, 'delete_dealer.html', {'dealer': dealer})
