...
class purchase2(models.Model):
    ...
    book_title = models.ForeignKey(book2, on_delete=models.CASCADE, default=None)
    dealers = models.ForeignKey(dealers2, on_delete=models.CASCADE, default=None)
    author = models.ForeignKey(book2, on_delete=models.CASCADE, related_name='purchases', null=True, blank=True)
    orderdate = models.DateField()
    bookquantity = models.PositiveIntegerField()
    perbookprice = models.PositiveIntegerField()
    totalprice = models.IntegerField(default=0,blank=True)
    p_list = (("cash", "cash"), ("net-banking", "net-banking"), ("UPI", "UPI"), ("Card", "Card"))
    ...
    def str(self):
        return f"{self.book_title}-{self.dealers}-{self.book_title.author} {self.get_book_pub_date} "

    def save(self, *args, **kwargs):
        # Calculate total price based on quantity and per-book price
        self.totalprice = self.bookquantity * self.perbookprice

        # You can now use book_pub_date in your logic or save it to another field if needed
        # For example, you can save it to a field in the purchase2 model:
        self.book_pub_date = self.book_title.pub_date
# Try to fetch the corresponding Stock instance based on the book and dealer associated with the purchase
        try:
            stock = BookStock2.objects.get(book=self.book_title, dealer=self.dealers)
            stock.quantity_in_stock += self.bookquantity  # Increase stock quantity
            stock.save()
